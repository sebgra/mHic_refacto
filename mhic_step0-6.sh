...
projectPath="/home/sardine/Bureau/mHiC" # $1 ## path to save all the raw data, intermediate files and outputs.
...
id="TROPHOZOITES" # $2 - sample name equivalent
fastqPath="$projectPath/fastqFiles" #  $3 fastq reads folder 
...
## step 1: Alignment
## ******************
name="TROPHOZOITES" # $4 sample name equivalent
ref="$projectPath/bin/PlasmoDB-9.0_Pfalciparum3D7_Genome.fasta" # $5 sample name equivalent (same as $2)
bwaDir="$(which bwa)" ##"$projectPath/Softwares/bwa-0.5.9" ## need downloading bwa software
samtoolsDir="$(which samtools)" ## "$projectPath/Softwares/samtools-1.3" ## need dowloading samtools software
fastqDir="$projectPath/fastqFiles"
resultsDir="$projectPath/$name"
bin="$projectPath/bin"
nCores=8 # $6
seqLength=25 # $7
resolution=10000 # $8 bin size
saveFiles=0 # $9
cutsite="GATCGATC" # $10 ## for MboI ##"AAGCTAGCTT" for HindIII or (GATCGATC ******** ********) for multiple cutters.
