...
do
    ...
    readL="$(cat $fastqDir/$name\_$i.fastq | head -2 | awk '{if(NR%4==2) print length($1)}')"

    echo "Step1.1 - BWA alignment"
    $bwaDir aln -n 2 -o 1 -q 0 -t $core $ref $fastqDir/$name\_$i.fastq >$resultsDir/$name\_$i.sai
	...
	$samtoolsDir view -h -f 4  $resultsDir/$name\_$i.sam >$resultsDir/$name\_unmapped_$i.sam
	...
	$samtoolsDir fastq $resultsDir/$name\_unmapped_$i.sam >$resultsDir/$name\_unmapped_$i.fastq
	...
	$bwaDir aln -n 2 -o 1 -q 0 -t $core $ref $resultsDir/$name\_unmapped_trim_filter_$i.fastq >$resultsDir/$name\_unmapped_trim_filter_$i.sai
	$bwaDir samse -n 99 $ref  $resultsDir/$name\_unmapped_trim_filter_$i.sai $resultsDir/$name\_unmapped_trim_filter_$i.fastq >$resultsDir/$name\_unmapped_trim_filter_$i.sam
	$samtoolsDir view $resultsDir/$name\_unmapped_trim_filter_$i.sam >$resultsDir/$name\_unmapped_trim_filter_noheader_$i.sam
