...
def get_args():
    ...
    parser.add_argument(
        "-f", "--fragment", help="Restriction Enzyme(RE) fragment file.", default=None
    )
    parser.add_argument(
        "-r", "--read", help="Paired-ended read alignment file.", default=None
    )
    parser.add_argument(
        "-o",
        "--outdir",
        help="Output directory to save category results.",
        default=None,
    )
    parser.add_argument(
        "-l",
        "--lower",
        help="Lower bound of the read pair distances summation from read end alignment position to its assigned RE fragment cutting site. Default value is None indicating no restriction.",
        default=None,
    )
    parser.add_argument(
        "-u",
        "--upper",
        help="Upper bound of the read pair distances summation from read end alignment position to its assigned RE fragment cutting site. Default value is None indicating no restriction.",
        default=None,
    )
    parser.add_argument(
        "-d",
        "--distance",
        help="Minimum distance between intrachromosomal read pair alignments positions. Default value is None indicating no restriction.",
        default=None,
    )
    parser.add_argument(
        "-m",
        "--method",
        help="Binning methods: by fixed number of window size, i.e. window, or fixed number of RE fragment, i.e. fragment.",
        default="fragment",
    )
    parser.add_argument(
        "-b",
        "--bin",
        help="Binning resolution. If binning by fixed number of window size, it can be 10000 or any other suitable bin size. If binning by fixed number of RE fragment, it can be 10 representing 10RE fragments or other suitable bin size.",
        default=None,
    )
    parser.add_argument(
        "-s",
        "--summary",
        help="(Optional) Summary of categories. Default is true.",
        default=True,
    )
    parser.add_argument(
        "-sf",
        "--summaryFile",
        help="(Optional) Summary file path. Default is alignedFileName.readCategorySummary.",
        default=None,
    )
    parser.add_argument(
        "-v", "--verbose", help="(Optional) Verbose. Default is true.", default=True
    )
    ...
    if chrom in resFrag:
        ...
        if len(resfrag) > 1:
            print(
                "Warning : "
                + len(resfrag)
                + " restriction fragments found for "
                + read.qname
                + "- skipped"
            )
            return None
            ...
            print(
                "Warning - no restriction fragments for "
                + read.qname
                + " at "
                + chrom
                + ":"
...
def load_restriction_fragment(
    in_file, minfragsize=None, maxfragsize=None, verbose=False
):
    """
    This function cites the same function from Hi-C Pro (https://github.com/nservant/HiC-Pro/blob/master/scripts/mapped_2hic_fragments.py) by Nicolas Servant, Eric Viara
    """
    ...
    for line in bed_handle:
        ...
        try:
            ...
            print(
                "Warning : wrong input format in line" + nline + ". Not a BED file !?"
            )
            continue
        ...
        if chromosome in resFrag:
            ...
            tree.add_interval(
                Interval(start, end, value={"name": name, "midPoint": midPoint})
            )
        else:
            tree = Intersecter()
            tree.add_interval(
...
def get_overlapping_resFragBin(binTree, chrom, read):
    ...
    if chrom in binTree.keys():
        ...
        print(
            "Warning - no bin found for " + read.qname + " at " + chrom + ":" + str(pos)
        )
        return None
...
def get_interaction_type(
    read1, read1_chrom, resfrag1, read2, read2_chrom, resfrag2, verbose
):
    """
    Function cite the same function from Hi-C Pro (https://github.com/nservant/HiC-Pro/blob/master/scripts/mapped_2hic_fragments.py) by Nicolas Servant, Eric Viara
    """
    ...
    if (
        ...
        and resfrag1 is not None
        and resfrag2 is not None
    ):
...
def get_valid_pair(
    r1, r2, r1_chrom, r2_chrom, r1_frag, r2_frag, r1_bin, r2_bin, cisDist
):
...
if __name__ == "__main__":
    ...
    if any(args.method in x for x in fragMethodList):
        binTree = build_resFragBin_tree(
            in_file=args.fragment, resolution=args.bin, verbose=args.verbose
        )
    else:
        binTree = None
    ...
    if args.read.endswith(".bam"):
        ...
        print(
            "Input paired-ended alignment file should either be in bam format or sam format."
        )
        sys.exit()
            ...
            if r1 is not None and r2 is not None:
                ...
                for end1 in m1:
                    for end2 in m2:
                        ...
                        end1_frag = get_overlapping_restriction_fragment(
                            fragTree, end1_chrom, end1
                        )
                        if end1_frag is None:
                            interactionType = "DUMP"
                            break

                        end2_frag = get_overlapping_restriction_fragment(
                            fragTree, end2_chrom, end2
                        )
                        ...
                        if end1_frag is not None and end2_frag is not None:
                            ...
                            fragDist = get_PE_fragment_size(
                                end1, end2, end1_frag, end2_frag, interactionType
                            )
                            cisDist = get_cis_dist(end1, end2)
                            # If the alignment position to fragment cutting sites distance sum is out of range allowed
                            if (
                                args.lower is not None
                                and fragDist is not None
                                and fragDist < int(args.lower)
                            ) or (
                                args.upper is not None
                                and fragDist is not None
                                and fragDist > int(args.upper)
                            ):
                                interactionType = "DUMP"
                            ...
                            if (
                                interactionType == "VI"
                                and args.distance is not None
                                and cisDist is not None
                                and cisDist < int(args.distance)
                            ):
                                interactionType = "DUMP"

                            if (
                                interactionType == "VI"
                            ):  # if this potential contact is valid - write out
                                valid_count += 1
                                # get the overlapped bin info
                                if binTree != None:
                                    end1_bin = get_overlapping_resFragBin(
                                        binTree, end1_chrom, end1
                                    )
                                    end2_bin = get_overlapping_resFragBin(
                                        binTree, end2_chrom, end2
                                    )
                                    mid1 = end1_bin.value["midPoint"]
                                    mid2 = end2_bin.value["midPoint"]
                                elif binWindow != None:
                                    mid1 = int(
                                        int(get_read_pos(end1)) / binWindow
                                    ) * binWindow + int(binWindow / 2)
                                    mid2 = int(
                                        int(get_read_pos(end2)) / binWindow
                                    ) * binWindow + int(binWindow / 2)
                                else:
                                ...
                                if end1_chrom != end2_chrom or mid1 != mid2:
                                    ...
                                    fragPair.append(
                                        [
                                            end1.qname,
                                            end1_frag.value["name"],
                                            end2_frag.value["name"],
                                        ]
                                    )
                                    # valid interactions cover how many distinct bins
                                    binPair.append(
                                        [end1.qname, end1_chrom, mid1, end2_chrom, mid2]
                                    )
                                else:
                                    interactionType = "DUMP"
                ...
                if len(m1) == 1 and len(m2) == 1:  # uni-read pair
                    ...
                    if (
                        valid_count == 1
                    ):  # indicating multi-mapping read pair has unique valid contact
                        multi_uv_count += 1
                        outContactNum.write(
                            "\t".join([r1.qname, "MultiToUni", "1"]) + "\n"
                        )
                    elif valid_count == 0:
                        md_count += 1
                    else:
                        multi_mv_count += 1
                        outContactNum.write(
                            "\t".join([r1.qname, "Multi", str(valid_count)]) + "\n"
                        )
    ...
    if args.summary:
        if args.summaryFile is None:
            outSummary = open(
                args.outdir + "/" + fileName + ".readCategorySummary", "w"
            )
        else:
        ...
        outSummary.write(
            "Total number of read pairs:\t" + str(uni_count + multi_count) + "\n"
        )
        outSummary.write("  Uniquely mapping read pairs:\t" + str(uni_count) + "\n")
        outSummary.write(
            "    Uniquely mapping valid read pairs:\t" + str(uv_count) + "\n"
        )
        outSummary.write(
            "    Uniquely mapping dangling end read pairs:\t" + str(de_count) + "\n"
        )
        outSummary.write(
            "    Uniquely mapping self cycle read pairs:\t" + str(sc_count) + "\n"
        )
        outSummary.write(
            "    Uniquely mapping religation read pairs:\t" + str(re_count) + "\n"
        )
        outSummary.write(
            "    Uniquely mapping dumped read pairs:\t" + str(ud_count) + "\n"
        )
        outSummary.write("  Multi-mapping read pairs:\t" + str(multi_count) + "\n")
        outSummary.write(
            "    Multi-mapping unique valid read pairs:\t" + str(multi_uv_count) + "\n"
        )
        outSummary.write(
            "    Multi-mapping multiple valid read pairs:\t"
            + str(multi_mv_count)
            + "\n"
        )
        outSummary.write(
            "    Multi-mapping multiple valid read pair but unique fragment pairs:\t"
            + str(multi_mv_uf_count)
            + "\n"
        )
        outSummary.write(
            "    Multi-mapping multiple valid read pair but unique bin pairs:\t"
            + str(multi_mv_ub_count)
            + "\n"
        )
        outSummary.write(
            "    Multi-mapping dumped read pairs:\t" + str(md_count) + "\n"
        )
