...
def get_args():
    ...
    parser.add_argument(
        "-f", "--file", help="Path to the interaction file to be read.", default=None
    )
    parser.add_argument(
        "-o", "--outdir", help="Path to directory for outputted files.", default=None
    )
    parser.add_argument(
        "-r",
        "--resolution",
        help="Resolution of the interaction, i.e., 40000.",
        type=int,
        default=None,
    )
    parser.add_argument(
        "-l", "--chrLens", help="Input file for chromosome lengths.", default=None
    )
    parser.add_argument(
        "-c",
        "--chrNum",
        help='Chromosome number to read in following format: chrNum. If whole genome reading is required, please use "whole".',
        default="whole",
    )
    parser.add_argument(
        "-tr",
        "--toRemove",
        help="Percentage of sparse row/columns to remove from matrix. Default is to remove top 10% sparse elements.",
        default=10,
        type=int,
    )
    parser.add_argument(
    ...
    if (
        args.file is None
        or args.outdir is None
        or args.resolution is None
        or args.chrLens is None
    ):
        parser.print_help()
        sys.exit()
    ...
    if args.toRemove <= 0 or args.toRemove >= 100:
        print(
            "Percentage of sparse rows/columns to be removed should be within (0, 100)."
        )
        sys.exit()
    ...
    with open(outFile, "w") as matrixFile:
        ...
        if chrNum == "whole":
            for i in range(len(row)):
                ...
                matrixFile.write(
                    ("%s\t%d\t%s\t%d\t%.4f\n")
                    % (chr1, int(mid1), chr2, int(mid2), values[i])
                )
        else:
            for i in range(len(row)):
                ...
                matrixFile.write(
                    ("%s\t%d\t%s\t%d\t%.4f\n")
                    % (chrNum, int(mid1), chrNum, int(mid2), values[i])
                )
        ...
        with open(biasFile, "w") as biasWOzero:
            if chrNum == "whole":
                for i in range(len(bias)):
                    ...
                    if bias[i, 0] != -1:
                        biasWOzero.write(
                            "\t".join([str(chr), str(int(mid)), str(bias[i, 0])]) + "\n"
                        )
                    biasWzero.write(
                        "\t".join([str(chr), str(int(mid)), str(bias[i, 0])]) + "\n"
                    )
                ...
                for i in range(len(bias)):
                    ...
                    if bias[i, 0] != -1:
                        biasWOzero.write(
                            "\t".join([str(chrNum), str(int(mid)), str(bias[i, 0])])
                            + "\n"
                        )
                    biasWzero.write(
                        "\t".join([str(chrNum), str(int(mid)), str(bias[i, 0])]) + "\n"
...
def normMatrix(rawMatrix, R, removePerc, revFragsDic):
    ...
    print(
        "## Raw matrix shape :"
        + str(rawMatrix.shape[0])
        + "*"
        + str(rawMatrix.shape[1])
    )
    st = time.time()
    ...
    print(
        "## Matrix shape after removing sparse bins :"
        + str(hic_mtx.shape[0])
        + "*"
        + str(hic_mtx.shape[1])
    )
    ...
    print(
        "## Matrix shape after normalization :"
        + str(norm_mtx.shape[0])
        + "*"
        + str(norm_mtx.shape[1])
    )
...
if __name__ == "__main__":
    ...
    R, hic_mtx = readInteraction(
        args.file, args.chrNum, args.resolution, chrLenDic, allFragsDic
    )
    ...
    print(
        "## Length of revFragsDic before sparse element removal:"
        + str(len(revFragsDicAll))
    )
    print(
        "## Length of revFragsDic after sparse element removal:" + str(len(revFragsDic))
    )
    ## 4. output normalized interaction
    writeInteraction(
        norm_mtx, baseName, args.outdir, revFragsDic, args.chrNum, args.resolution
    )
